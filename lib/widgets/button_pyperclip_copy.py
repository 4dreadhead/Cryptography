...
class ButtonPyperclipCopy:
    def __init__(self, obj, custom_coordinates=None):
        ...
        button.setFont(font)
        button.setObjectName("button_pyperclip_copy")
        button.setStyleSheet(UiHelper.default_style())

        button.setCursor(QtGui.QCursor(QtCore.Qt.CursorShape.PointingHandCursor))
        button.setText(translate("Object", "Копировать"))

        button.clicked.connect(lambda: self.copy(obj))
    ...
    @staticmethod
    def copy(obj):
        pyperclip.copy(obj.result)
        obj.statusbar.showMessage("Результат скопирован в буффер обмена.")
